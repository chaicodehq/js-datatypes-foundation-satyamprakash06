...
export function repeatPattern(pattern, times) {
    ...
    const repeated = pattern.repeat(times);
    return repeated;
}
...
export function extractRangoliCenter(design, start, end) {
   ...
   const sliced = design.slice(start, end);
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  const rongali = colorString.split(oldSep).join(newSep);
  return rangoli;
...
export function makeRangoliBorder(char, length) {
  ...
  const repeatedCahr = char.repeat(length+5);
  const slicedChar = repeatPattern.slice(0, length);
  return slicedChar;
}
